...
const SPANISH_PATH_ALIASES: Record<string, string> = {
  tienda: "store",
  carrito: "cart",
  producto: "products",
  pedido: "order",
...
export async function middleware(request: NextRequest) {
  ...
  if (first && SPANISH_PATH_ALIASES[first]) {
    const rest = segments.slice(1).join("/")
    const mapped = SPANISH_PATH_ALIASES[first]
    const target = rest ? `/${mapped}/${rest}` : `/${mapped}`
    const query = request.nextUrl.search || ""
    return NextResponse.redirect(
      `${request.nextUrl.origin}/es${target}${query}`,
      307
    )
  }

  if (first === "es" && segments[1] && SPANISH_PATH_ALIASES[segments[1]]) {
    const mapped = SPANISH_PATH_ALIASES[segments[1]]
